...
# Case sensitive and typo problems

Direct$Fecoccemp <- toupper(Direct$Fecoccemp)

library(magrittr)
library(RecordLinkage)
test = Direct %>% select(Fecoccemp,fectransid) 
...
Direct = Direct %>% select(-"Fecoccemp")
Direct = left_join(Direct,test,by = 'fectransid')

unique(Direct$Fecoccemp)
# After cleaning, 272 company names
